...
export class BookSqlModel {
    ...
    public static updateBook = async (id: string | number,title: string, price: number, authorId: number | string): Promise<string> => {
        return new Promise((resolve, reject) => {
            db.query(`UPDATE ${Tables.BOOKS} SET title=?, price=?, authorId=?
                    WHERE id=${id}`,[title,price,authorId], (err, res) => {
                if (err) {
                    return reject(err)
                }
                return resolve(null);
